...
class LGFX_Class : public lgfx::LGFX_Device
{
    lgfx::Panel_ST7789      _panel_instance;
    lgfx::Bus_SPI           _bus_instance;
    // Light_PWM removed — managed directly to avoid ledcAttachPin() glitch.

public:
    /* Call AFTER Lcd.init() + fillScreen() have written clean GRAM.
     * Attaches LEDC ch.7 to GPIO42. Any brief duty=0 during ledcAttachPin()
     * only reveals already-black GRAM — invisible to the user. */
    void initBacklight()
    {
        ledcSetup(7, 44100, 9);   /* 9-bit, matches LovyanGFX Light_PWM internals */
        ledcWrite(7, 512);        /* 100% HIGH = backlight OFF (active-low hardware) */
        ledcAttachPin(HAL_PIN_LCD_BL, 7);
    }

    /* Drop-in replacement for lgfx::LGFX_Device::setBrightness().
     * Uses identical formula to LovyanGFX Light_PWM::setBrightness()
     * (PWM_BITS=9, invert=true, offset=0) so all existing call-sites work. */
    void setBrightness(uint8_t brightness)
    {
        uint32_t duty = 0;
        if (brightness) {
            duty = (uint32_t)brightness * 257u + 64u;
            duty >>= 7;           /* >>= (16 - 9) */
        }
        duty = 512u - duty;       /* invert: active-low BL, setBrightness(255)=ON */
        ledcWrite(7, duty);
    }
...
/* Backlight managed via initBacklight() / setBrightness() above — not via Light_PWM. */
