...
ES8311::ES8311(I2C_Class* i2c, uint8_t i2cAddr){
    _i2c = i2c;
    _i2cAddr = i2cAddr;
...
bool ES8311::begin() {
    ...
    // I2C bus is already initialized by BSP (In_I2C.begin()),
    // just verify ES8311 is present on the bus
    if (!_i2c->scanID(_i2cAddr)) {
        log_e("ES8311 not found at 0x%02X", _i2cAddr);
        return false;
    }
    ...
    ok |= WriteReg(0x01, 0x3F);  // Enable all clocks, MCLK from MCLK pin (bit7=0)

    // Ensure slave mode + I2S format (clear bit6 of REG00)
    reg = ReadReg(0x00);
    reg &= 0xBF;  // bit6=0 → slave
    ...
    ok |= WriteReg(0x32, 0xBF); // DAC volume ~75% (avoid first-play silence)
...
bool ES8311::setSampleRate(uint32_t sample_rate){
    uint8_t reg = 0;
    bool ok = true;
    _mclk_hz = sample_rate * 128; // Audio library uses I2S_MCLK_MULTIPLE_128
    ...
    reg = ReadReg(0x02);
    reg &= 0x07;  // clear pre_div[7:5] and pre_multi[4:3] before writing
...
bool ES8311::setBitsPerSample(uint8_t bps){
    ...
    reg09 &= ~(0x07 << 2);  // clear bits [4:2] before writing
    reg0A &= ~(0x07 << 2);  // clear bits [4:2] before writing
    switch (bps) {
...
bool ES8311::WriteReg(uint8_t reg, uint8_t val){
	return _i2c->writeRegister8(_i2cAddr, reg, val);
}

uint8_t ES8311::ReadReg(uint8_t reg){
	return _i2c->readRegister8(_i2cAddr, reg);
}
